...
describe('Testing DOM related functions via jest-dom', () => {
  ...
  it('Unit test of the getElementIndex function', () => {
    expect(
      getElementIndex(child_1)
    ).toBe(0);
    expect(
      getElementIndex(child_2)
    ).toBe(1);
    expect(
      getElementIndex(child_3)
    ).toBe(2);
  });
  ...
  it('Integration test of the elementQueryCount function', () => {
    expect(
      elementQueryCount('.root')
    ).toBe(1);
    expect(
      elementQueryCount('.child')
    ).toBe(3);
    expect(
      elementQueryCount(
        '.invalid-selector'
  ...
  it('Integration test of the makeValidClass function', () => {
    ...
    expect(
      makeValidClass(rootDetails)
    ).toBe('.parent');
    expect(
      makeValidClass(preElementDetails)
    ).toBe('');
  });
  ...
  it('Integration test of the reduceElementSelector function', () => {
    const rootDetails: ElementDetails =
      getElementDetails(rootElement);
    const childDetails: ElementDetails =
      getElementDetails(child_1);
    const article: ElementDetails =
      getElementDetails(articles);

    expect(
      reduceElementSelector(rootDetails)
    ).toBe('.parent');
    expect(
      reduceElementSelector(
        childDetails
