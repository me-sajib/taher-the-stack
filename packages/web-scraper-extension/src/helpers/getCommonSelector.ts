...
const getCommonSelector = (
  ...
  let ft: ElementFootprint | null =
    footprint;
  const checkQueryCount = predictQuery(
    selectorsMap,
    2
  );
  ...
  const parentSuggested: string =
    selectorsMap.get(
      Math.min(...selectorsMap.keys())
    )!;
  const suggest: string =
    reduceQueryString(
      `${parentSuggested} > ${reduceElementSelector(
        footprint
      )}`
    );
  const selected =
    getExactSelector(footprint);

  return {
    selected,
    parentSuggested,
    prioritize: getPrioritySelector(
      suggest,
      footprint.position
    ),
    suggest,
