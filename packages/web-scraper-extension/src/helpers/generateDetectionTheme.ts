...
import { HSL, StyleSchema } from '../interfaces/extension';
...
const createColorVariants = (
  ...
  paginated: createHsl(colorPalate.exceptCurrentItem)
});

const generateDetectionTheme = (color: HSL): StyleSchema[] => {
  const colorVariants = createColorVariants(color);

  return detectionStyles.map((schema: StyleSchema) => {
    const value = schema.attr?.value ?? '';
