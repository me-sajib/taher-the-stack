...
const makeResultSchema = (
  ...
  for (const uid in scrapeSelectors) {
    const { name, selectors }: Select = scrapeSelectors[uid];
    const suggestSelectors: string[] = [];

    for (const key in selectors) {
      const selector: Selector = selectors[key];

      if (selector.customSelects.length) {
        suggestSelectors.push(...selector.customSelects);
      } else {
        suggestSelectors.push(selector.suggest);
      }
    }
    ...
    if (name in resultSchema) {
      resultSchema[
        generatePerfectKey<ResultSchema>(resultSchema, name)
      ] = suggestSelectors;
    } else {
      resultSchema[name] = suggestSelectors;
    }
  }
...
const scrapeElement = (element: Element): string | MultiResult => {
  if (!element) {
    return '';
  }
  ...
  const isMultiValue: boolean = dataValues.filter(Boolean).length > 1;

  if (isMultiValue) {
    // filtering the empty result
    return Object.entries(data).reduce((acc, [key, value]) => {
      if (value) {
        acc[key as keyof MultiResult] = value;
      }
      return acc;
    }, {} as MultiResult);
  }
...
const resultScraper = (resultSchema: ResultSchema): Result[] => {
  const resultElements: Map<string, Element[]> = Object.entries(
    resultSchema
  ).reduce((map, [propertyName, suggestSelector]) => {
...
export { makeResultSchema, resultScraper };
