...
import { ProxyModalData } from 'packages/proxy-dashboard/interfaces';
import store from 'packages/proxy-dashboard/store';
import { isAuthorize } from 'packages/proxy-dashboard/utils';
import qs from 'qs';
...
export const deleteProxy = createAsyncThunk(
  ...
  async (payload: { proxyListKey: string; proxyIds: number[] }) => {
    try {
      await axios.delete(
        `${PROXY_URL}/delete?${qs.stringify(payload, {
          encode: false
        })}`
      );
