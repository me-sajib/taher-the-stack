...
const SelectorButtons = ({
  ...
  const hasValidUid: boolean = Boolean(currentSelector.uid);
  const isPrevCurrentCountSame: boolean =
    prevSaveCount === scrapeSelectorList.length;

  const currentBubbleTransition = useTransition(!hasValidUid, {
    from: { scale: 0 },
    enter: [{ scale: 0 }, { scale: 1 }],
    config: {
      duration: 250
    },
  ...
  return (
    <div className="selector-buttons">
      {Children.toArray(
        scrapeSelectorList.map(({ uid, totalCount, color }) => {
          const isSelected = currentSelector.uid === uid;
          ...
          return (
            <CountBubble
              ...
              styles={Object.assign(
                {
                  backgroundColor: createHsl(color)
                },
                isSelected ? bounce : { opacity: 0.55 }
              )}
              isSelected={isSelected}
              resultCount={
                hasValidUid && isSelected
        ...
        (styles, invalidUid) =>
          invalidUid && (
            <CountBubble
              styles={Object.assign(
                styles,
                isPrevCurrentCountSame && bounce,
                {
                  backgroundColor: createHsl(colorPalate.current)
                }
              )}
              uid={currentSelector.uid}
              resultCount={currentSelector.totalCount}
            />
          )
      )}
      ...
      <Spring
        ...
        reset={prevSaveCount < scrapeSelectorList.length}
      >
        {(styles: any) => (
          <animated.div style={styles}>
            <Button
              ...
              clickAction={saveSelectorHandler}
            />
          </animated.div>
        )}
      </Spring>

      {Boolean(currentSelector.totalCount) && (
        <Button
          classes={'scraper-button'}
          status={'clear'}
          clickAction={clearButtonHandler}
        />
      )}
    </div>
  );
