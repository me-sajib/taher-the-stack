...
export class ManageHandler {
  private MAX_RETRIES: number = Number(
  ...
  *retries(cb) {
  ...
  handleRequest() {
    ...
    const clientReqHandlers =
      createEventHandlers(
        this.clientReq
      );

    this.EVENTS.slice(0, -2).forEach(
      (event) => {
        this.req.on(
          event,
          clientReqHandlers[event]
        );
      }
    );
  ...
  handleSocket() {
    console.log(
      `Socket Request ${this.req.method}: ${this.req.url}`
    );
    const socketEvents =
    ...
    this.clientReq
      ...
      .on('connect', (_, socket) => {
        ...
        const socketRequestHandlers =
          createEventHandlers(this.res);
